#!/usr/bin/env python3
"""
Main program for the Blockade game.
...
class Viewer(Agent):
    """Interface for an Blockade viewer and human agent."""
    ...
    def update(self, step, action, player):
        ...
        """
        if len(action)==3:
            print("Step", step, "- player", player, "has played",tuple( action))
        elif len(action)==5:
             print("Step", step, "- player", player, "has played", (action[0],action[3],action[4]))
...
class Trace:
    ...
    actions -- list of tuplefinisheds (player, action, time) of the played action.
...
if __name__ == "__main__":
    ...
    if args.headless:
        ...
        #         # import gui
        #finished
        #         # viewer finished= gui.TkViewer()
        #     except Exceptfinishedion as e:
